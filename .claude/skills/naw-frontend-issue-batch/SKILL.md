...
- **このスキルはIssue起票のみを行う**。ブランチ作成・`docs/issue-X/`生成・実装・PR作成は行わない（着手は別途 `naw-frontend-issue-workflow` または `naw-frontend-issue-workflow` ベースの他スキルに委ねる）
...
自分自身がこのスキルの実行中に `loop` から再入場したものでなければ（＝ユーザーが `/naw-frontend-issue-batch` を直接叩いた最初の起動であれば）、真っ先に以下を行う。

```
Skill(skill="loop", args="/naw-frontend-issue-batch")
```

これにより `loop` スキルが自己ペースモードで起動し、以降は1サイクル完了ごとに `ScheduleWakeup` で次回起動が予約され、`/naw-frontend-issue-batch` のサイクル本体が繰り返し実行される。
...
- `Agent(subagent_type: naw-frontend-explore)` を呼び、以下を自己完結のプロンプトで依頼する（会話の前提を知らない前提で、必要な情報をすべてプロンプトに含めること）
  ...
  - 複数の独立した候補単位がある場合は、依存するバックエンドAPIが実装済みで・かつ粒度が小さいものを1件だけ選ぶこと
  - 結果は「候補の有無」「単位（画面・コンポーネント一覧）」「対応する移植元ファイルパス（Angular）」「依存するバックエンドAPIの実装状況」「粒度をこうした理由」「選定理由」を簡潔に返すよう指示すること
- `naw-frontend-explore` の調査結果を受け取り、メインループ側で次のいずれかを行う
  - 候補あり: 「3. Issue起票」へ進む
  - **候補なし**: ループを停止する（`ScheduleWakeup(stop: true)` を呼ぶ）。ユーザーに「移植可能な候補が見つからなかったため停止した（起票件数: N件）」旨を報告して終了する

### 3. Issue起票

`naw-frontend-issue-workflow` の「Issue起票手順」に従う（`gh issue create --label "frontend-port"` → `gh project item-add`）。**`frontend-port` ラベルの付与を忘れないこと。** 承認は不要。ブランチ作成（`start_issue.sh`）・ドキュメント生成・実装は行わない。
...
- 10未満の場合: 起票結果を簡潔に報告したうえで、`ScheduleWakeup` で次回起動を予約する（間隔は目安として60〜120秒程度。`naw-frontend-explore`呼び出しのコストを踏まえ、キャッシュ窓を意識しつつ選ぶ）。`prompt` には `<<autonomous-loop-dynamic>>` を渡す
