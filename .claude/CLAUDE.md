...
`main` ブランチ上で直接実装作業を行わない。Issue 対応を開始する際は必ず `feature/#XX` ブランチを作成してから作業すること。

- 新規ブランチは基本的に `develop` から切る（`git checkout -b feature/#XX develop`）
- `main` からは切らない

### Issue 対応開始時の確認手順

1. `git branch --show-current` で現在のブランチを確認する
2. 対象 Issue の `feature/#XX` ブランチが存在しない場合、依存する未マージ PR がないかを `gh pr list` で確認する
3. 依存がなければ `develop` から、依存があればその `feature/#YY` から切る
...
実装前に以下のドキュメントを順番に作成すること。置き場所は `docs/#XX/`（git 管理対象）。
...
ドキュメントは `docs/#XX/` に git 管理されているため、PR Description から参照してもよい。
