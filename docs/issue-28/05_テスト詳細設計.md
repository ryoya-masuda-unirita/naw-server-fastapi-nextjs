...
セッション後のテーブルクリア対象に `tenant_endpoints` を追加する。
...
## `backend/tests/integration/test_tenant_endpoints.py`（新規）
...
async def admin_user(session, tenant):
    """テスト用テナント管理者"""
    u = User(
        id=uuid4(), tenant_id=tenant.id, login_id="admin-test", name="Admin",
        role=UserRole.ADMIN, is_required_password_reset=False,
    )
    session.add(u)
    await session.commit()
...
async def normal_user(session, tenant):
    """テスト用一般ユーザー"""
    u = User(
        id=uuid4(), tenant_id=tenant.id, login_id="user-test", name="User",
        role=UserRole.USER, is_required_password_reset=False,
    )
    session.add(u)
    await session.commit()
    await session.refresh(u)
...
async def local_server_endpoint(session, tenant):
    ...
    e = TenantEndpoint(
        tenant_id=tenant.id, type=EndpointType.LOCAL_SERVER,
        endpoint_name="Local", endpoint="http://localhost:8080", api_key="secret-key",
    )
    session.add(e)
    await session.commit()
...
async def azure_endpoint(session, tenant):
    ...
    e = TenantEndpoint(
        tenant_id=tenant.id, type=EndpointType.AZURE_OPENAI_CHAT,
        endpoint_name="Azure", endpoint="https://example.openai.azure.com", api_key="azure-key",
    )
    session.add(e)
    await session.commit()
    await session.refresh(e)
...
class TestGetEndpoints:
    ...
    async def test_tenant_admin_gets_all_types(self, client, admin_headers, tenant, local_server_endpoint, azure_endpoint):
...
class TestGetEndpointsByType:
    ...
    async def test_returns_only_matching_type(self, client, admin_headers, local_server_endpoint, azure_endpoint):
        """指定タイプのエンドポイントのみ返ること"""
        async with client as c:
...
class TestUpdateEndpoint:
    ...
    async def test_rejects_update_on_non_local_server_endpoint(self, client, admin_headers, tenant, azure_endpoint):
    ...
    async def test_rejects_type_change_to_non_local_server(self, client, admin_headers, tenant, local_server_endpoint):
...
class TestDeleteEndpoint:
    ...
    async def test_rejects_delete_on_non_local_server_endpoint(self, client, admin_headers, tenant, azure_endpoint):
