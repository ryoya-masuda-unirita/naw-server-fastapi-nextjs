...
| `backend/app/core/security.py` | 変更（追加対応） | `get_current_user` の `credentials` 引数に型ヒントを付与 |
| `backend/app/routers/auth.py` | 変更（追加対応） | `logout()` に戻り値型ヒントを付与 |
| `backend/app/models/password_history.py` | 変更（追加対応） | `Optional[datetime]` を `datetime \| None` に変更 |
| `backend/app/schemas/auth.py` | 変更（追加対応） | `Optional[str]` を `str \| None` に変更（2箇所） |

実装中にユーザーから「コードベース全体の型アノテーションを確認してほしい」との追加依頼があり、`backend/app/` 配下を全件調査した。`UserResponse.from_user`（指摘9番）以外に、型ヒント欠落2箇所・`Optional`記法違反3箇所が見つかったため、本Issueのスコープ内で合わせて修正した。
